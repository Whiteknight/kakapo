...
INIT {
    ...
    my %from_meths := pir::inspect__PPS($from_nsp, "methods");

    for %pmcs {
        my $pmc_name := ~ $_;
        my $to_nsp := Parrot::get_hll_namespace($pmc_name);
        my %meth_hash := pir::inspect__PPS($to_nsp, "methods");

        # If meth_hash is null, it means that the proxy has already
        # been created and pulled all the methods in. In that case, inject
        # the methods into the Class directly. Otherwise, add them to the
        # namespace
        if pir::isnull__IP(%meth_hash) {
            my $to_class := $to_nsp.get_class();
            for %pmcs{$_} {
                my $method := %from_meths{$_};
